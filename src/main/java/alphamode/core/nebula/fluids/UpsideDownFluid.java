...
/**
 * Represents a fluid which can flow upside down.
 */
public abstract class UpsideDownFluid extends Fluid {
    public static final BooleanProperty FLYING;
    ...
    protected boolean method_15749(BlockView world, BlockPos pos, Direction direction) {
        ...
        if (fluidState.getFluid().matchesType(this)) {
            ...
            return blockState.getMaterial() != Material.ICE && blockState.isSideSolidFullSquare(world, pos, direction);
        }
    }
        ...
        if (!state.isEmpty()) {
            ...
            if (this.canFlow(world, fluidPos, blockState, Direction.UP, blockPos, blockState2, world.getFluidState(blockPos), fluidState.getFluid())) {
                ...
                if (this.method_15740(world, fluidPos) >= 3) {
                    this.spreadSides(world, fluidPos, state, blockState);
                }
    ...
    private void spreadSides(WorldAccess world, BlockPos pos, FluidState fluidState, BlockState blockState) {
        int i = fluidState.getLevel() - this.getLevelDecreasePerBlock(world);
        if (fluidState.get(FLYING)) {
            i = 7;
        }
        ...
        if (i > 0) {
            ...
            for (Map.Entry<Direction, FluidState> directionFluidStateEntry : map.entrySet()) {
                ...
                FluidState fluidState2 = directionFluidStateEntry.getValue();
    ...
    public FluidState getFlowing(int level, boolean falling) {
        return this.getFlowing().getDefaultState().with(LEVEL, level).with(FLYING, falling);
    }

    public abstract Fluid getStill();

    public FluidState getStill(boolean falling) {
        return this.getStill().getDefaultState().with(FLYING, falling);
    }
            ...
            if (direction2 != direction) {
                ...
                if (this.canFlowThrough(world, this.getFlowing(), blockPos, blockState, direction2, blockPos3, blockState2, fluidState)) {
                    boolean bl = short2BooleanMap.computeIfAbsent(s, (ix) -> {
                        ...
                        BlockState blockState2x = world.getBlockState(blockPos4);
    ...
    protected static int getBlockStateLevel(FluidState state) {
        return state.isStill() ? 0 : 8 - Math.min(state.getLevel(), 8) + (state.get(FLYING) ? 8 : 0);
    }
    ...
    public VoxelShape getShape(FluidState state, BlockView world, BlockPos pos) {
        return state.getLevel() == 9 && isFluidAboveEqual(state, world, pos) ? VoxelShapes.fullCube() : this.shapeCache.computeIfAbsent(state, (fluidState) -> VoxelShapes.cuboid(0.0D, 0.0D, 0.0D, 1.0D, fluidState.getHeight(world, pos), 1.0D));
    }

    static {
        FLYING = NebulaProperties.FLYING;
